...
class YOLOv1Net(object):
    ...
    def model_save(self):
        self.model.save('yolo_v1_model.h5')
    ...
    def loss(self, labels, predicts):
        ...
        ④不包含目标的边界框置信度误差。
        ⑤类别预测误差。
        :param predicts:    网络预测，2-D张量，[batch_size,(7*7*(2*5+20))] 每一个预测的形状(7*7*(2*5+20))
        :param labels:      真实标签
        :param scope:
        :return:
        """
        ...
        class_loss = tf.reduce_mean(tf.reduce_sum(tf.square(class_dalta), axis=[1,2,3]), name="class_loss") * self.class_scale
        # ②
        # 包含目标的置信度损失，平方和误差
        ...
        coord_lose = tf.reduce_mean(tf.reduce_sum(tf.square(boxes_delta), axis=[1,2,3,4]), name="coord_lose") * self.coord_scale

        tf.summary.scalar("class_loss", class_loss)
        tf.summary.scalar("object_loss", object_loss)
        tf.summary.scalar("noobject_loss", noobject_loss)
        tf.summary.scalar("coord_loss", coord_lose)
        ...
        return class_loss + object_loss + noobject_loss + coord_lose
